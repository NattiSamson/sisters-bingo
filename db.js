...
const  u2  = await pool.query('SELECT count(id) FROM payment_accounts WHERE is_active = TRUE AND RIGHT(account_number,4) = RIGHT($1,4)', [receipt.creditedPartyAccountNo]);
   ...
   return 3;
	  ...
	  console.log('aaaaaa ' + id);
		...
		await pool.query('INSERT INTO deposits(user_id,payment_account_id,deposit_method_id,depositor_name,depositor_account,amount,amount_after,reference,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NOW()) RETURNING id', [rows[0].id,1,1,receipt.creditedPartyName,receipt.creditedPartyAccountNo,receipt.settledAmount.replace(/[^0-9.]/g, ""),rows[0].balance + receipt.settledAmount.replace(/[^0-9.]/g, ""),receipt.receiptNo]);
		return 4;
      },
